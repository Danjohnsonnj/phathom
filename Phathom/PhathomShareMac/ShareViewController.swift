...
private struct ShareRootView: View {
                ...
                if imageJPEGData == nil, provider.hasItemConformingToTypeIdentifier(UTType.image.identifier) {
                    do {
                        ...
                        if let img = obj as? NSImage {
                            imageJPEGData = jpegForLibrary(from: img)
    ...
    private func jpegForLibrary(from image: NSImage) -> Data? {
        guard let tiff = image.tiffRepresentation,
              let rep = NSBitmapImageRep(data: tiff),
              let raw = rep.representation(using: .jpeg, properties: [.compressionFactor: 0.9])
        else { return nil }
        return MediaImageEncoding.normalizedJPEGForLibraryStorage(from: raw) ?? raw
    }
}

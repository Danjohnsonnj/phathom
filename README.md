...
- **Highlights & notes** — In detail **source**, select text to create a **highlight** (persisted in SwiftData); optional **per-highlight note**. Anchors use the same markdown-to-plain rules as the stripper (versioned on the model so offsets stay consistent if stripping changes).

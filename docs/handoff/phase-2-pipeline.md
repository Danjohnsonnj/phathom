...
**Title preservation**: After a successful scrape, **`BackgroundPipeline`** populates **`item.title`** from `result.suggestedListTitle` / `result.pageTitle` **only when `item.titleUserSet == false`**. **`AddNewTab`** sets the flag when the user types a title at capture; **`DetailView`**'s inline **`TextField`** sets it on commit/blur (and clears it when the user empties the field, allowing a future scrape to repopulate). **`ShareCapture.insertWebItem`** and the share extension leave the flag `false` so page-title scraping behaves as before.
...
Phathom needs three distinct outputs from the same article text. Keep **one model-loaded session** per item (avoids reloading the GGUF three times). The implementation **prefills the article once** using **KV cache prefix reuse** (`llama_memory_seq_cp`): **article-first** prompts share an identical token prefix; each task forks that KV state, decodes its instruction suffix, generates, then drops the fork. **`analyzeArticle`** + **`ModelSession.analyze`** checkpoint after each task. If the combined path hits **`contextLimitReached`** or the stub runtime is active, the pipeline **falls back** to three sequential **`generate*`** calls with per-task adaptive token fitting. Informal device benchmark: **~60s → ~30s** cold-start analyze for the same article on **iPhone 16 Pro** (model/article dependent); see README and **`[PhathomPipeline]`** logs.
...
- **Inference**: `LlamaCppRuntime` + **`LlamaContentAnalyzer`** (templated user prompts for summarize / tags / extracts; JSON array parsing with graceful nil fields). **KV prefix path**: article-first prompts + **`generateWithSharedPrefix`** / **`analyzeArticle`**; context params include **`n_seq_max`**, Flash Attention **AUTO**, **`offload_kqv`**, **`kv_unified`**, **`n_ubatch`** (see `RuntimeConfig`). Tag prompts ask for lowercase kebab-case JSON arrays (2–5 subject + 1–2 content-type tags); **`TagNameNormalizer`** in PhathomCore applies before SwiftData upsert for both LLM tags and merged Instagram/TikTok hashtags. **Speed tuning** (sequential `generate*` paths): tag prompts use the first **~4k** chars of article text and **maxTokens 96**; extract prompts use **~8k** chars; summary still uses **~12k** (see code). **`PipelineMetrics`** prints per-stage wall times to the Xcode console (`scrape`, `load_model`, `summarize`, `tags_llm`, `tag_db`, `extracts_llm`) prefixed with `[PhathomPipeline]` — remove `PipelineMetrics.swift` and wrappers to drop logging.
...
- Changing any existing SwiftData model properties **except** those explicitly extended in **`docs/decisions.md`** (currently **`archivedAt`**, **`sourceMarkdown`**, and **`titleUserSet`** on **`ContentItem`**)
